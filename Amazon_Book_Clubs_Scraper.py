...
def scrape_AmazonBookClubs(path):

    start = time.time()
    print('-'*75)
    print('Scraping AmazonBookClubs.com ...')
    print('-'*75)
    ...
    if path == '':
        name = 'AmazonBookClubs_data.xlsx'
        # getting the books under each category
        links = []
        nbooks = 0
        homepage = 'https://www.amazon.com/s?i=stripbooks&rh=n%3A8081575011&fs=true&qid=1679861000'
        driver.get(homepage)
        while True:           
            # scraping books urls
            ...
            for title in titles:
                try:
                    ...
                    print('The below error occurred during the scraping from AmazonBookClubs.com, retrying ..')
                    print('-'*50)
                    print(err)
                    continue
            ...
            try:
                button = wait(driver, 2).until(EC.presence_of_element_located((By.CSS_SELECTOR, "a[class='s-pagination-item s-pagination-next s-pagination-button s-pagination-separator']")))
        ...
        with open('AmazonBookClubs_links.csv', 'w', newline='\n', encoding='utf-8') as f:
            writer = csv.writer(f)
    ...
    if path != '':
        ...
        df_links = pd.read_csv('AmazonBookClubs_links.csv')
                    ...
                    for li in lis:
                        ...
                        if 'Publication date' not in details:
                            try:
                                ...
                                for month in months:
                                    if month in text:
                                        details['Publication date'] = ' '.join(text.split(',')[1:])
                                        break
                            except:
                                pass
    ...
    print('-'*75)
    print(f'AmazonBookClubs.com scraping process completed successfully! Elapsed time {elapsed} mins')
    print('-'*75)
    driver.quit()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        path = sys.argv[1]
    data = scrape_AmazonBookClubs(path)
